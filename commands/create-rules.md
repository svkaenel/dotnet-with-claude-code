...
| Web App (Frontend) | Blazor Webassembly/React/Angular/Svelte, no server code |
| API/Backend | ASP.NET Controllers/ASP.NET Minimal Apis/Express/etc, no frontend |
| Library/Package | .NET Assembly,  `main`/`exports` in package.json, publishable |
| CLI Tool | .NET Commandline Application, `bin` in package.json, command-line interface |
...
From project configuration files (*.csproj, package.json, etc.), identify:
- Runtime/Language (.NET, Node, Bun, Deno, browser)
- Framework(s)
- Database (if any)
- Testing tools
- Build tools
...
**Output path**: `CLAUDE.md` (project root), .claude/references for references
...
**Important**: Keep CLAUDE.md concise, if it's getting too long to be easily consumed, then split parts of the information into single files in .claude/references folder and link them with a description in CLAUDE.md. Goal is to consume only the minimum context memory as possible. This is especially valid for information which cover specific features for the project.
...
4. Optionally create reference docs in `.claude/references/`
```
...
- Split into multiple files if it's getting too long
